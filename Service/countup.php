...
if(isset($_GET['id']) > 0)
{
    $idFolder = "./data/" . $_GET['id'];
    if(file_exists($idFolder) == FALSE)
    {
        mkdir($idFolder);
    }

    // カウンターファイル名
    $CounterFileName = $idFolder . "/Counter.xml";; 

    //　排他スタート
    $lockfilename = $idFolder . "/lock"; //　排他用のファイル名
    ...
    if (file_exists($CounterFileName) == FALSE) 
    {
        $xml_text = <<<XML
        <Counter>
        <Sec/>
